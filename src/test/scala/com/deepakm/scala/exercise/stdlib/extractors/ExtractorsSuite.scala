package com.deepakm.scala.exercise.stdlib.extractors
...
class ExtractorsSuite extends AnyFunSuite {
  test("When you create a case class, it automatically can be used with pattern matching since it has an extractor: ") {
  ...
  test("What's an extractor? In Scala it's a method in any object called unapply, and that method is used to disassemble the object given by returning a tuple wrapped in an option. Extractors can be used to assign values:") {
  ...
  test("An extractor can also be used in pattern matching: ") {
    class Car(val make: String, val model: String, val year: Short, val topSpeed: Short)
  ...
  test("Since we aren't really using u and v in the previous pattern matching, they can be replaced with _: ") {
    class Car(val make: String, val model: String, val year: Short, val topSpeed: Short)
  ...
  test("As long as the method signatures aren't the same, you can have as many unapply methods as you want: ") {
    class Car(val make: String, val model: String, val year: Short, val topSpeed: Short)
  ...
  test("A custom extractor is typically created in the companion object of the class. In this exercise, we use it as an assignment: ") {
    class Employee(
  ...
  test("In this exercise we use unapply for pattern matching employee objects: "){
    class Employee(
                    val firstName: String,
                    val middleName: Option[String],
                    val lastName: String)
